...
```bash
sudo apt-get update && sudo apt-get install -y zlib1g-dev libcurl4-openssl-dev libssl-dev build-essential
```
...
COPY --chown=dev:dev . /workspace/a-curl-library
RUN mkdir -p /workspace/build/a-curl-library && \
    cd /workspace/build/a-curl-library && \
    cmake /workspace/a-curl-library && \
